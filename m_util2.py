            ...
            for phase in ['train']:
                ...
                for batch_index, (inputs, labels) in enumerate(dataloaders):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        #print('dataset sizes',dataset_sizes[phase], 'dataset sizes')
                        
                        #print(labels.shape,'labels')
                        _, preds = torch.max(outputs, 1)
                        outputs = torch.squeeze(outputs) #Added here to try to fix resnext, but giving odd behav now
                        loss = criterion(outputs, labels) #### EXPERIMENT BELOW###
